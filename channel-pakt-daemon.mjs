...
CORE
  ...
  -u, --url <url>            Paktsuite API base URL        [PAKTSUITE_URL, default: http://localhost:9000]
...
const PAKTSUITE_URL       = cli.url                ?? process.env.PAKTSUITE_URL       ?? 'http://localhost:9000';
...
if (cli['openclaw-container']) process.env.OPENCLAW_CONTAINER = cli['openclaw-container'];
...
const INVITE_AGENT_ADDRESS = cli['invite-address']  ?? process.env.INVITE_AGENT_ADDRESS ?? '';
const JOB_TITLE            = cli['job-title']        ?? process.env.JOB_TITLE            ?? 'Agent-to-Agent Task';
const JOB_DESCRIPTION      = cli['job-description']  ?? process.env.JOB_DESCRIPTION      ?? 'A task created programmatically by an agent buyer.';
...
const JOB_DELIVERABLE      = cli['job-deliverable']   ?? process.env.JOB_DELIVERABLE      ?? 'Send the buyer a message confirming job acceptance and your readiness to deliver.';

if (!agentKey)     { console.error('--key / AGENT_PRIVATE_KEY is required'); process.exit(1); }
if (!agentAddress) { console.error('--address / AGENT_ADDRESS is required'); process.exit(1); }
...
const REVIEWED_PATH = cli['reviewed-path'] ?? process.env.REVIEWED_PATH ?? `/tmp/daemon-${agentName}-reviewed.json`;
...
const APPLIED_PATH = cli['applied-path'] ?? process.env.APPLIED_PATH ?? `/tmp/daemon-${agentName}-applied.json`;
...
function checkTurns(convId) {
  ...
  if (t) clearTimeout(t);
  timers.set(convId, setTimeout(() => { turns.delete(convId); timers.delete(convId); }, TURN_RESET_MS));
  return n <= MAX_TURNS;
}
...
async function signAndBroadcast(txPayload) {
  ...
  if (!rpcUrl) throw new Error(`No RPC URL configured for chain ${txPayload.chainId}`);
...
async function scanAndApply(sdk, currentUserId) {
  let listed;
  try {
    listed = sdkOk(await sdk.job.list({ status: 'open', limit: 20 }), 'job.list (scan)');
  } catch (err) {
  ...
  const eligible = jobs.filter(j => {
    ...
    return creator !== currentUserId   // not own job
      && receiver !== currentUserId    // not already working it
      && !appliedJobs.has(jobId);      // not already applied or invited
  });

  if (eligible.length === 0) return;
  console.log(`[${agentName}] [SCAN] ${eligible.length} open job(s) eligible for application`);
...
async function readWalletBalance(coin, address) {
  ...
  if (isToken && coin?.contractAddress) {
    ...
    return { formatted: ethers.formatUnits(raw, Number(coin.decimal ?? '18')), symbol };
  }
...
async function callAnthropic(message) {
  ...
  if (!clientOpts.apiKey && !clientOpts.authToken) {
    throw new Error('ANTHROPIC_API_KEY or CLAUDE_AUTH_TOKEN is required for anthropic sandbox');
  }
  ...
  for (let attempt = 0; attempt <= 4; attempt++) {
    try {
      ...
      if (status === 429 && attempt < 4) {
        ...
        console.log(`[${agentName}] Rate limited — retrying in ${delay}ms (attempt ${attempt + 1}/4)`);
...
async function callHermes(message) {
  ...
  if (!res.ok) throw new Error(`Hermes responded ${res.status}: ${await res.text()}`);
...
async function generateReply(message) {
  switch (SANDBOX_TYPE) {
    ...
    default: throw new Error(`Unknown SANDBOX_TYPE "${SANDBOX_TYPE}" — must be anthropic | openclaw | hermes`);
  }
}
...
async function resolveUserIdByAddress(address) {
  const res = await fetch(`${PAKTSUITE_URL}/v1/account-public/by-wallet/${encodeURIComponent(address)}`);
...
async function createJobAndInvite(sdk, inviteeAddress) {
  ...
  console.log(`[${agentName}] Escrow address: ${depositData?.escrowAddress} — amount: ${depositData?.coinAmount} ${depositData?.coinSymbol}`);
  ...
  if (depositData?.approve) {
    ...
    console.log(`[${agentName}] Approve tx broadcast — txHash: ${approveTxHash}`);
  ...
  if (depositData?.deposit) {
    ...
    console.log(`[${agentName}] Deposit tx broadcast — txHash: ${depositTxHash} — waiting for confirmation...`);
  ...
  for (let attempt = 1; attempt <= 6; attempt++) {
    try {
      ...
      validated = true;
      console.log(`[${agentName}] Escrow funded and validated (attempt ${attempt})`);
      break;
    } catch (err) {
      console.log(`[${agentName}] validatePayment attempt ${attempt}/6 failed: ${err.message}`);
  ...
  // ── Step 6: Send invite ───────────────────────────────────────────────────
  console.log(`[${agentName}] Inviting ${inviteeAddress} (userId: ${inviteeUserId}) to job ${jobId}...`);
...
function isMessagingDeliverable(deliverable) {
  const text = `${deliverable.title ?? ''} ${deliverable.description ?? ''}`.toLowerCase();
  return (
...
async function executeJob(sdk, messaging, jobId) {
  if (executingJobs.has(jobId)) {
    console.log(`[${agentName}] executeJob: already processing job ${jobId} — skipping duplicate`);
    return;
  }
...
async function _runJob(sdk, messaging, jobId) {
  ...
  if (jobSeller && jobSeller !== agentAddress.toLowerCase()) {
    console.log(`[${agentName}] Skipping job "${job.title}" — not the seller (seller: ${jobSeller})`);
    return;
  }

  const allDeliverables = job.deliverables ?? [];
  const pending = allDeliverables.filter(d => d.status !== 'completed');
  console.log(`[${agentName}] Job "${job.title}" — ${pending.length}/${allDeliverables.length} deliverable(s) pending`);
  ...
  for (const deliverable of pending) {
    const taskText = [
      ...
      deliverable.description ? `Deliverable details: ${deliverable.description}` : '',
    ...
    try {
      ...
      console.error(`[${agentName}] Deliverable LLM failed for "${deliverable.title}":`, err.message);
      continue;
    }
        ...
        try {
          const convo = await Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(() => reject(new Error('createDirectConversation timed out after 10s')), 10_000)
            ),
          ]);
          messaging.sendMessage({ conversationId: convo._id, type: 'TEXT', message: response });
          console.log(`[${agentName}] Message sent to creator in conversation ${convo._id}`);
        } catch (err) {
          console.error(`[${agentName}] Failed to send message to creator:`, err.message);
        }
      } else {
        console.warn(`[${agentName}] Messaging deliverable but creator ID unknown — skipping send`);
      }
    }

    try {
      sdkOk(
        await sdk.job.toggleDeliverableProgress(jobId, String(deliverable._id), { status: 'completed' }),
        'toggleDeliverableProgress'
      );
      console.log(`[${agentName}] Deliverable "${deliverable.title}" marked complete`);
    } catch (err) {
      console.error(`[${agentName}] Failed to mark deliverable complete:`, err.message);
    }
  }

  const refreshed    = sdkOk(await sdk.job.getById(jobId), 'getById (pre-complete check)');
  const refreshedJob = refreshed?.job ?? refreshed;
  const stillPending = (refreshedJob?.deliverables ?? []).filter(d => d.status !== 'completed');
  if (stillPending.length > 0) {
    console.error(`[${agentName}] ${stillPending.length} deliverable(s) still incomplete — skipping completeJob`);
    return;
  }

  console.log(`[${agentName}] All deliverables confirmed complete — completing job`);
  ...
  if (markReadyPayload) {
    console.log(`[${agentName}] Signing markReady tx for chain ${markReadyPayload.chainId}...`);
    try {
      const txHash = await signAndBroadcast(markReadyPayload);
      console.log(`[${agentName}] markReady broadcast — txHash: ${txHash} — waiting for confirmation...`);

      let confirmed = false;
      for (let attempt = 1; attempt <= 6; attempt++) {
        await new Promise(r => setTimeout(r, 10_000));
        try {
          sdkOk(await sdk.job.confirmTx(jobId, { step: 'onMarkReady', txHash }), 'confirmTx onMarkReady');
          console.log(`[${agentName}] Job marked ready on-chain (attempt ${attempt}) — txHash: ${txHash}`);
          confirmed = true;
          break;
        } catch (err) {
          console.log(`[${agentName}] confirmTx onMarkReady attempt ${attempt}/6 failed: ${err.message}`);
        }
      }
      if (!confirmed) {
        console.error(`[${agentName}] confirmTx onMarkReady exhausted retries — buyer cannot release until daemon restarts`);
      }
    } catch (err) {
  ...
  console.log(`[${agentName}] Awaiting buyer payment release — review will be submitted on next connect`);
}

// ── Post-job review ────────────────────────────────────────────────────────

async function reviewCompletedJobs(sdk) {
  const listResult = await sdk.job.list({ status: 'completed', role: 'seller', limit: 20 });
  ...
  for (const j of mine) {
    ...
    if (!buyerId) {
      console.log(`[${agentName}] Skipping review for "${j.title}" — buyer ID unknown`);
      continue;
    }
    ...
    let rating = 5;
    let review = 'Great experience. Clear requirements and smooth collaboration throughout.';
    try {
      ...
      if (match) {
        ...
        if (Number.isInteger(parsed.rating) && parsed.rating >= 1 && parsed.rating <= 5) rating = parsed.rating;
        if (parsed.review) review = parsed.review;
      }
    } catch (err) {
      console.log(`[${agentName}] Review generation failed, using default:`, err.message);
    }

    console.log(`[${agentName}] Submitting review for "${j.title}" — ${rating}/5 — receiverId: ${buyerId}`);
    console.log(`[${agentName}] Review text: ${review}`);
    try {
      const reviewData = sdkOk(
        await sdk.job.submitReview(jobId, { receiverId: buyerId, review, rating }),
        'submitReview'
      );
      reviewedJobs.add(jobId);
      saveReviewed();
      console.log(`[${agentName}] Review submitted for "${j.title}" — id: ${reviewData?._id}`);
    } catch (err) {
      console.error(`[${agentName}] submitReview failed for "${j.title}":`, err.message);
    }
  }
}
...
async function handleJobInvite(sdk, messaging, invite) {
  ...
  console.log(`[${agentName}] JOB_INVITE received — job: ${jobTitle} (${jobId}), inviteId: ${inviteId}`);
  ...
  try {
    const raw = await generateReply(prompt);
    decision = raw.trim().toLowerCase().startsWith('accept') ? 'accept' : 'decline';
  } catch (err) {
    console.error(`[${agentName}] Job invite reasoning failed, defaulting to decline:`, err.message);
    decision = 'decline';
  }
  ...
  try {
    ...
    console.log(`[${agentName}] Accepted job invite ${inviteId} — acceptPayload present: ${!!acceptData?.acceptPayload}`);

    if (acceptData?.acceptPayload) {
      console.log(`[${agentName}] Signing acceptPayload for chain ${acceptData.acceptPayload.chainId}...`);
      ...
      console.log(`[${agentName}] No on-chain accept needed — invite accepted off-chain`);
    }
...
async function run() {
  console.log(`[${agentName}] Starting — sandbox: ${SANDBOX_TYPE} — address: ${agentAddress}`);
  ...
  while (true) {
    try {
      ...
      console.log(`[${agentName}] Connected to paktsuite — listening for messages`);
      ...
      messaging.onJobInvite((invite) => {
        ...
        console.log(`[${agentName}] [WS] onJobInvite fired — inviteId: ${invite?.inviteId} jobId: ${inviteJobId}`);
        // Mark this job so the scan loop doesn't also apply to it.
        if (inviteJobId) { appliedJobs.add(String(inviteJobId)); saveApplied(); }
        handleJobInvite(sdk, messaging, invite).catch(err =>
          console.error(`[${agentName}] handleJobInvite uncaught:`, err.message)
        );
      });
      ...
      messaging.onPaymentReleased(async (event) => {
        ...
        const txHash   = event?.escrowReleaseTxHash ?? event?.data?.escrowReleaseTxHash;

        console.log(`[${agentName}] [WS] PAYMENT RELEASED — "${jobTitle}" — ${amount}`);
        ...
        let job = null;
        try {
          const jobData = sdkOk(await sdk.job.getById(jobId), 'getById (payment check)');
          job = jobData?.job ?? jobData;
        } catch (err) {
          console.error(`[${agentName}] [WS] Could not fetch job for balance check:`, err.message);
        }
        ...
        if (asset && asset !== '0x0000000000000000000000000000000000000000') {
          try {
            const { formatted, symbol } = await readTokenBalance(asset, chainId, agentAddress);
            console.log(`[${agentName}] [WS] Token balance after release: ${formatted} ${symbol} (contract: ${asset})`);
          } catch (err) {
            console.error(`[${agentName}] [WS] Token balance check failed:`, err.message);
          }
        }
        ...
        try {
          ...
          console.log(`[${agentName}] [WS] Wallet balance after release: ${ethers.formatEther(raw)} ${nativeSym}`);
        } catch (err) {
          console.error(`[${agentName}] [WS] Native balance check failed:`, err.message);
        }
        ...
        if (job && !reviewedJobs.has(jobId)) {
          ...
          if (buyerId) {
            ...
            try {
              const raw = await generateReply(prompt);
              console.log(`[${agentName}] [WS] Review LLM raw: ${raw?.slice(0, 200)}`);
              const match = raw.match(/\{[\s\S]*?\}/);
              if (match) {
                const parsed = JSON.parse(match[0]);
                if (Number.isInteger(parsed.rating) && parsed.rating >= 1 && parsed.rating <= 5) rating = parsed.rating;
              ...
              console.log(`[${agentName}] [WS] Review generation failed, using default:`, err.message);
            }

            console.log(`[${agentName}] [WS] Submitting review for "${job.title}" — ${rating}/5 — receiverId: ${buyerId}`);
            try {
              const reviewData = sdkOk(
                await sdk.job.submitReview(jobId, { receiverId: buyerId, review, rating }),
                'submitReview'
              ...
              reviewedJobs.add(jobId);
              saveReviewed();
              console.log(`[${agentName}] [WS] Review submitted — id: ${reviewData?._id}`);
            } catch (err) {
              console.error(`[${agentName}] [WS] submitReview failed:`, err.message);
            }
          } else {
      ...
      messaging.onJobApplied(async (event) => {
        ...
        console.log(`[${agentName}] [WS] APPLICATION RECEIVED — "${jobTitle}" — from ${applicantId}`);
        if (coverLetter) console.log(`[${agentName}] [WS] Cover letter: "${coverLetter.slice(0, 120)}..."`);

        if (!jobId || !applicationId) {
          console.warn(`[${agentName}] [WS] Missing jobId or applicationId — cannot accept`);
          return;
        }
        ...
        try {
          ...
          console.log(`[${agentName}] [WS] Application decision generation failed, defaulting to accept`);
        }

        if (!accept) {
          console.log(`[${agentName}] [WS] Decided NOT to accept application for "${jobTitle}"`);
          return;
        }
        ...
        try {
          ...
          console.log(`[${agentName}] [WS] Accepted application for "${jobTitle}" — invite auto-created for ${applicantId}`);
        } catch (err) {
          console.error(`[${agentName}] [WS] acceptApplication failed:`, err.message);
        }
      });
      ...
      messaging.onJobApplicationAccepted(async (event) => {
        const jobId    = event?.jobId    ?? event?.data?.jobId;
        const jobTitle = event?.jobTitle ?? event?.data?.jobTitle ?? jobId;
        console.log(`[${agentName}] [WS] APPLICATION ACCEPTED — "${jobTitle}" — awaiting invite to start work`);
      });
      ...
      messaging.onJobCompleted(async (event) => {
        ...
        console.log(`[${agentName}] [WS] JOB COMPLETED — "${jobTitle}" — seller: ${sellerId} — releasing payment`);
        if (!jobId) return;

        // Fetch full job to get escrowChainId and receiver (seller) ID.
        let job = null;
        try {
          const jobData = sdkOk(await sdk.job.getById(jobId), 'getById (completion)');
          job = jobData?.job ?? jobData;
        } catch (err) {
          console.error(`[${agentName}] [WS] Could not fetch job on completion:`, err.message);
          return;
        }

        // Abort if job isn't in review status (already released or duplicate event).
        if (job?.status !== 'review') {
          console.log(`[${agentName}] [WS] Job "${jobTitle}" status is "${job?.status}" — skipping release`);
          return;
        }
        ...
        try {
          ...
          console.log(`[${agentName}] [WS] Release tx broadcast — txHash: ${txHash}`);
          await new Promise(r => setTimeout(r, 8_000));
          ...
          console.log(`[${agentName}] [WS] Payment released — job "${jobTitle}"`);
        } catch (err) {
          console.error(`[${agentName}] [WS] releasePayment failed:`, err.message);
          return;
        }

        // Submit review of seller after releasing payment.
        const receiverId = String(job?.receiver?._id ?? job?.receiver ?? sellerId);
        if (receiverId && !reviewedJobs.has(jobId)) {
          ...
          let rating = 5;
          let review = 'Excellent work. Delivered exactly as described and communicated clearly.';
          try {
            const raw = await generateReply(prompt);
            const match = raw.match(/\{[\s\S]*?\}/);
            if (match) {
          ...
          console.log(`[${agentName}] [WS] Submitting review for seller — ${rating}/5 — receiverId: ${receiverId}`);
          try {
            const reviewData = sdkOk(
            ...
            reviewedJobs.add(jobId);
            saveReviewed();
            console.log(`[${agentName}] [WS] Seller review submitted — id: ${reviewData?._id}`);
          } catch (err) {
            console.error(`[${agentName}] [WS] submitReview (seller) failed:`, err.message);
          }
        }
      });

      messaging.onJobReview((event) => {
        const receiverId = String(event?.receiverId ?? event?.data?.receiverId ?? '');
        ...
        if (receiverId === userId) {
          console.log(`[${agentName}] [WS] Review RECEIVED — ${rating}/5 — from ${ownerId} — job: ${jobId}`);
          console.log(`[${agentName}] [WS] Review text: "${review}"`);
        } else if (ownerId === userId) {
          console.log(`[${agentName}] [WS] Own review confirmed — ${rating}/5 for ${receiverId} — job: ${jobId}`);
        } else {
          console.log(`[${agentName}] [WS] job_review event (role unclear):`, JSON.stringify(event)?.slice(0, 200));
        }
      });
      ...
      messaging.onBroadcast(async (msg) => {
        ...
        if (!checkTurns(msg.conversation)) {
          console.log(`[${agentName}] Turn limit reached for conv ${msg.conversation}`);
          return;
        }

        console.log(`[${agentName}] Received message from ${msg.user}: ${msg.content?.slice(0, 80)}`);
        ...
        messaging.sendMessage({ conversationId: msg.conversation, type: 'TEXT', message: reply });
      ...
      try {
        ...
        const pending = (inviteList?.data ?? []).filter(i => i.direction === 'received' && i.status === 'pending');
        if (pending.length > 0) {
          console.log(`[${agentName}] Found ${pending.length} pending invite(s) on connect — processing`);
        ...
        console.error(`[${agentName}] Failed to poll pending invites:`, err.message);
      }
      ...
      try {
        ...
        console.error(`[${agentName}] Failed to review completed jobs:`, err.message);
      }

      // Scan released payments and log on-chain balance per token
      try {
        const paidResult = await sdk.job.list({ status: 'completed', role: 'seller', limit: 20 });
        const paidJobs   = paidResult.data?.data ?? paidResult.data?.jobs ?? [];
        const released   = paidJobs.filter(j =>
          (j.escrowStatus === 'released' || j.escrowStatus === 'completed') &&
          (j.seller ?? '').toLowerCase() === agentAddress.toLowerCase()
        );
        if (released.length > 0) {
          console.log(`[${agentName}] Payments received (${released.length} job(s)):`);
          ...
          for (const j of released) {
            ...
            console.log(`[${agentName}]   "${j.title}" — ${j.amount ?? '?'} ${symbol} | tx: ${j.escrowReleaseTxHash ?? 'n/a'}`);
          }
          // Token balance for each unique ERC-20 used as payment
          for (const coin of uniqueTokens.values()) {
            try {
              const { formatted, symbol } = await readWalletBalance(coin, agentAddress);
              console.log(`[${agentName}] Token balance: ${formatted} ${symbol}`);
            } catch (err) {
              console.error(`[${agentName}] Token balance check failed:`, err.message);
            }
          }
          ...
          for (const chainId of uniqueChains) {
            try {
              ...
              console.log(`[${agentName}] Wallet balance: ${ethers.formatEther(raw)} ${symbol}`);
            } catch (err) {
              console.error(`[${agentName}] Native balance check failed:`, err.message);
            }
          }
        }
      } catch (err) {
      ...
      try {
        const reviewsResult = await sdk.job.getReceivedReviews(userId, { limit: 10 });
        ...
        if (reviews.length > 0) {
          ...
          for (const r of reviews) {
            ...
            console.log(`[${agentName}]   ${r.rating}/5 from ${from}: "${r.review}"`);
          }
        }
      } catch (err) {
        console.error(`[${agentName}] Failed to fetch received reviews:`, err.message);
      }

      // Resume ongoing jobs with pending deliverables
      try {
        const jobListResult = await sdk.job.list({ status: 'ongoing', role: 'seller', limit: 20 });
        const ongoingJobs   = jobListResult.data?.data ?? jobListResult.data?.jobs ?? [];
        const resumable     = ongoingJobs.filter(j => {
          const isSeller     = !j.seller || (j.seller ?? '').toLowerCase() === agentAddress.toLowerCase();
          const deliverables = j.deliverables ?? [];
          return isSeller && deliverables.length > 0 && deliverables.some(d => d.status !== 'completed');
        });
        if (resumable.length > 0) {
          console.log(`[${agentName}] Found ${resumable.length} ongoing job(s) with pending deliverables — resuming`);
          for (const j of resumable) {
            if (!markProcessed(`job:${j._id}`)) continue;
            executeJob(sdk, messaging, String(j._id)).catch(err =>
              console.error(`[${agentName}] executeJob (resume) failed:`, err.message)
            );
          }
        }
      } catch (err) {
        console.error(`[${agentName}] Failed to poll ongoing jobs:`, err.message);
      }
      ...
      if (INVITE_AGENT_ADDRESS && !buyerJobCreated) {
        ...
        createJobAndInvite(sdk, INVITE_AGENT_ADDRESS).catch(err =>
          console.error(`[${agentName}] createJobAndInvite failed:`, err.message)
        );
      }

      // Seller mode: scan for public open jobs and apply immediately, then on a timer.
      scanAndApply(sdk, userId).catch(err =>
        console.error(`[${agentName}] Initial scanAndApply failed:`, err.message)
      );
      ...
      console.error(`[${agentName}] Connection error:`, err.message, '— retrying in 10s');
      clearInterval(scanTimer);
      try { messaging?.disconnect(); } catch {}
      await new Promise(r => setTimeout(r, 10_000));
    }
